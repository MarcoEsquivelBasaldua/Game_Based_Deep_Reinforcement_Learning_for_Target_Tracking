...
class Environment:
    ...
    def reset_fixed_initial_position(self):
        # En1 simulations
        #self.state = np.array([3.50, 6.50, 3.14, 5.84])
        #self.state = np.array([1.50, 4.50, 1.48, 4.35])

        # Env3 simulaions
        #self.state = np.array([11.50, 11.50, 12.27, 12.97])
        self.state = np.array([8.50, 4.50, 7.17, 3.29])
    ...
    def test_configurations(self, samples_per_evader_position, dist):
        initial_configurations = []
        ...
        for e in self.evader_actions.keys():
            for _ in range(samples_per_evader_position):
                ...
                while not self.isVisual(state) or not np.linalg.norm(e-p) <= dist:
                    p = np.random.rand(2)
                    p[0] *= self.Width
                    p[1] *= self.Length
                    state = np.array([e,p]).flatten()
